  ...
  {% call statement('period_boundaries', fetch_result=True) -%}
    with data as (
      select
          ...
          coalesce(
            {{dbt_utils_sqlserver.dateadd('millisecond',
                                -1,
    ...
    select
      start_timestamp,
      stop_timestamp,
      {{dbt_utils_sqlserver.datediff('start_timestamp',
                           'stop_timestamp',
...
{% materialization insert_by_period, default -%}
  ...
  {% set _ = dbt_utils_sqlserver.get_period_boundaries(schema,
                                              identifier,
                                              timestamp_field,
                                              start_date,
                                              stop_date,
  ...
  {% for i in range(num_periods) -%}
    ...
    {{ dbt_utils_sqlserver.log_info(msg) }}

    {%- set tmp_identifier = model['name'] ~ '__dbt_incremental_period' ~ i ~ '_tmp' -%}
    {%- set tmp_relation = api.Relation.create(identifier=tmp_identifier,
                                               schema=schema, type='table') -%}
    {% call statement() -%}
      {% set tmp_table_sql = dbt_utils_sqlserver.get_period_sql(target_cols_csv,
                                                       sql,
                                                       timestamp_field,
                                                       period,
                                                       start_timestamp,
